// Condensed source code for the Rotman Interactive Trader Client API
// Obtained, combined, and annotated by Ted Hilk, fall 2012
//
// This file contains (respectively):
//
//  1) The decompiled source code for the RIT Client API
//      - accepts commands from Excel VBA, or from any .NET language
//      - used to place orders, get order information, get cash balance, etc.
//      - *basically acts like a broker API for competitors*
//
//  2) The decompiled source code for the RIT Excel RTD Server
//      - NOTE: this is *not* the "Rotman Server"
//          - Rotman Server: runs the matching engine and runs centrally
//          - RTD Server: part of the Rotman Excel Link and runs locally
//      - integrates with Excel
//      - can be used to request various data from the Rotman Server
//      - allows running any command the Excel RTD link supports
//      - see Rotman Excel RTD Link documentation for command syntax
//      - *only used for getting information not available via the API*
//
// How to use these files:
//
//  1) Import the *.tlb files; see my email in the "Documentation" folder.
//
//  2) Both the API and the RTD server communicate with the RIT Client itself
//     via Windows named pipes (see in source code below), so it is not a
//     requirement that we include these specific *.dll files in the Visual
//     Studio project for the basic interface that will be provided to
//     competitors.  We could just copy in the necessary parts of the code
//     below.
//
// Enjoy!
...
namespace TTS
{
	...
	public class API : IAPI
	{
		...
		private void Open()
		{
			try
			{
				this.Proxy = new ChannelFactory<IAPIObject>(new NetNamedPipeBinding(),
															new EndpointAddress("net.pipe://localhost/Rotman/RIT/API")).CreateChannel();
